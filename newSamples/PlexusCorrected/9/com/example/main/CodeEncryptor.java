...
public class CodeEncryptor {
  ...
  public String decryptDecorated(String cipherText, char[] password) throws Exception {
    ...
    if ( isEncryptedString( cipherText ) ) {
      return Dec( unDecorate( cipherText ), password );
    }
    return Dec(cipherText,password);
  }
  ...
  private Cipher init(char[] password, byte[] slt, byte[] iv, boolean encrypt) throws Exception {
    int mode = encrypt ? Cipher.ENCRYPT_MODE : Cipher.DECRYPT_MODE;
    PBEKeySpec keySpec = new PBEKeySpec(password,slt, ITERATION_COUNT,KEY_SIZE);;
  ...
  public String Dec(String cipherText, char[] password) throws Exception {
    ...
    Cipher dc = init(password,slt,iv,false);
  ...
  public String Enc(String data, char[] password) throws Exception {
    byte[] slt = genSalt();
    byte[] iv = generateIV();
    Cipher encryptSipher = init(password,slt,iv,true);
  ...
  public String encryptAndDecorate(String data, char[] password) throws Exception {
     return decorate( Enc( data, password ) );
  }
